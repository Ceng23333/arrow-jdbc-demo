...
import com.example.MockPreparedStatement;
...
public class JdbcParameterBinderTest {
  ...
  void binary() throws SQLException {
    testSimpleType(
        ...
        Types.VARBINARY,
        (VarBinaryVector vector, Integer index, byte[] value) -> vector.setSafe(index, value),
        BaseVariableWidthVector::setNull,
  ...
  void largeBinary() throws SQLException {
    testSimpleType(
        ...
        Types.LONGVARBINARY,
        (LargeVarBinaryVector vector, Integer index, byte[] value) -> vector.setSafe(index, value),
        BaseLargeVariableWidthVector::setNull,
        Arrays.asList(new byte[0], new byte[] {2, -4}, new byte[] {0, -1, 127, -128}));
  }
  ...
  void decimal128() throws SQLException {
    testSimpleType(
        new ArrowType.Decimal(/*precision*/ 12, /*scale*/ 3, 128),
        Types.DECIMAL,
        DecimalVector::setSafe,
        DecimalVector::setNull,
        Arrays.asList(
  ...
  void decimal256() throws SQLException {
    testSimpleType(
        new ArrowType.Decimal(/*precision*/ 12, /*scale*/ 3, 256),
        Types.DECIMAL,
        Decimal256Vector::setSafe,
        Decimal256Vector::setNull,
        Arrays.asList(
            new BigDecimal("120.429"), new BigDecimal("-10590.123"), new BigDecimal("0.000")));
  }
  ...
  void listOfDouble() throws SQLException {
    TriConsumer<ListVector, Integer, Double[]> setValue =
    ...
    List<Double[]> values =
        Arrays.asList(
            new Double[] {0.0, Math.PI}, new Double[] {1.1, -352346.2, 2355.6},
    ...
    TriConsumer<ListVector, Integer, Boolean[]> setValue =
        (listVector, index, values) -> {
          ...
          Arrays.stream(values)
              .forEach(booleanValue -> writer.bit().writeBit(booleanValue ? 1 : 0));
          writer.endList();
          listVector.setLastSet(index);
        };
    List<Boolean[]> values =
  ...
  void listOfString() throws SQLException {
    TriConsumer<ListVector, Integer, String[]> setValue =
        (listVector, index, values) -> {
          org.apache.arrow.vector.complex.impl.UnionListWriter writer = listVector.getWriter();
          writer.setPosition(index);
          writer.startList();
  ...
  void mapOfString() throws SQLException {
    TriConsumer<MapVector, Integer, Map<String, String>> setValue =
        (mapVector, index, values) -> {
    ...
    List<Map<String, String>> values =
        Arrays.asList(value1, value2, value3, Collections.emptyMap());
    testMapType(
        new ArrowType.Map(true), setValue, MapVector::setNull, values, new ArrowType.Utf8());
  }

  @Test
  void mapOfInteger() throws SQLException {
    TriConsumer<MapVector, Integer, Map<Integer, Integer>> setValue =
        (mapVector, index, values) -> {
          org.apache.arrow.vector.complex.impl.UnionMapWriter mapWriter = mapVector.getWriter();
          mapWriter.setPosition(index);
          mapWriter.startMap();
    ...
    value3.put(0, 4096);
    List<Map<Integer, Integer>> values =
        Arrays.asList(value1, value2, value3, Collections.emptyMap());
    testMapType(
        new ArrowType.Map(true), setValue, MapVector::setNull, values, new ArrowType.Int(32, true));
  }
  ...
  <T, V extends FieldVector> void testSimpleType(
      ArrowType arrowType,
      int jdbcType,
